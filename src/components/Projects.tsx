...
export default function Projects({ locale = "ar", as = "h2" }: { locale?: Locale; as?: "h1" | "h2" }) {
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {t.projects.items.map((project, i) => (
            <div
              key={i}
              className="group glass-card rounded-2xl overflow-hidden hover:border-road-500/30 transition-all duration-500 hover:-translate-y-1 opacity-100 translate-y-0"
            >
              <div className="relative aspect-[16/10] overflow-hidden">
                <Image
                  src={`/${project.images[0]}.jpeg`}
                  alt={`${project.title} - مشروع سفلتة طرق في السعودية`}
                  fill
              ...
              <div className="p-4 md:p-5 pt-2 md:pt-3">
                <p className="text-sm text-asphalt-400 leading-relaxed mb-4">
                  {project.description}
                </p>
                ...
                {(() => {
                  ...
                  return (
                    <div className="flex justify-start">
                      <Link
                        href={projectRoutes[i]}
                        className="text-xs font-extrabold text-road-400 hover:text-road-300 transition-colors flex items-center gap-1 bg-road-500/5 hover:bg-road-500/10 px-3 py-1.5 rounded-lg border border-road-500/10"
                      >
                        {t.lang === "ar" ? "تفاصيل دراسة الحالة ←" : "View Case Study →"}
                      </Link>
                    </div>
                  );
                })()}
